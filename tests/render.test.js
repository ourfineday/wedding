...
test("render: 확정 설정(기본)은 테마바 숨김 + minimal 적용", () => {
...
test("render: previewMode=true면 테마바 4버튼 노출", () => {
  const { els } = loadApp({ config: (c) => { c.previewMode = true; } });
...
test("render: ?theme=classic 이면 data-theme=classic (previewMode 무관)", () => {
